...
const Products = ({data,setBasket}) => {


    const handleBasket = (index) => {

        data[index].inBasket=true
        console.log(data[index].inBasket)
              ...
              return (
                <div key={index} className=" w-72 h-96 bg-slite-300 px-16 py-5 mx-8">
                  <div  className="w-72 h-80 bg-white-200 border-2 rounded px-8 py-5 group hover:bg-slate-500 transition-all duration-100 hover:text-white" >
                    ...
                    <button onClick={(e)=>handleBasket(index)} className=" w-24 h-8 bg-green-600 text-sm rounded group-hover:bg-green-500 px-2 ">Add to Cart</button>
                    </div>
                  </div>
                </div>
